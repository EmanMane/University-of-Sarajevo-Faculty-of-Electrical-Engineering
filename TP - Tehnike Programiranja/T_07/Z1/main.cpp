...
int main ()
{
    ...
    try{
        ...
        for(int i=0;i<n;i++){
            int min=i;
            for(int j=i+1;j<n;j++) if(std::strcmp(spisakrecenica[j],spisakrecenica[min])<0) min=j;
            char* temp=spisakrecenica[i];
            spisakrecenica[i]=spisakrecenica[min];
            spisakrecenica[min]=temp;
        }
