...
class Liga{
    ...
    void SacuvajStanje(std::string ime_fajla){
        std::ofstream izlazni_tok(ime_fajla,std::ios::binary);
        izlazni_tok.write(reinterpret_cast<char*>(&max_br_timova),sizeof(max_br_timova));
        izlazni_tok.write(reinterpret_cast<char*>(&broj_timova),sizeof(broj_timova));
        for(int i=0;i<broj_timova;i++){
    ...
    void AzurirajIzDatoteke(std::string ime_fajla){
        std::ifstream ulazni_tok(ime_fajla);
        ...
        for(;;){
            ...
            int x,y; char znak;
            ulazni_tok>>x>>znak>>y;
            if(!ulazni_tok.eof()) ulazni_tok.get();
            if(tim1.size()>20 || tim2.size()>20 || znak!=':') throw std::logic_error("Problemi pri citanju datoteke");
            try{
                RegistrirajUtakmicu(tim1.c_str(),tim2.c_str(),x,y);
            }catch(...){
    ...
    Liga(std::string ime_fajla){
        std::ifstream ulazni_tok(ime_fajla,std::ios::binary);
        if(!ulazni_tok) throw std::logic_error("Datoteka ne postoji");
        ulazni_tok.read(reinterpret_cast<char*>(&max_br_timova),sizeof(max_br_timova));
        ulazni_tok.read(reinterpret_cast<char*>(&broj_timova),sizeof(broj_timova));
